...
def train_input_fn(file_path):
    return dataset_input_fn(True, file_path, 32, None, 1)
...
def main(unused_arg):
    tf.flags.DEFINE_string('output_directory', 'dataset\\output', 'Output data directory')
    flags = tf.flags.FLAGS

    train_list, valid_list = image_utils.get_files_list(flags.output_directory)

    classifier = tf.estimator.Estimator(model_fn=cnn_model_fn, model_dir=os.path.join(flags.output_directory, "tb"))
